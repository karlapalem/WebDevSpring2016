...
module.exports = function(app, mongoose) {
  ...
  var api = {
    ...
    updateArtOfferedByUser: updateArtOfferedByUser,
    // Order
    updateOrderInfoForUser: updateOrderInfoForUser,
    updateSalesInfoForUser: updateSalesInfoForUser
  }
  return api;
  ...
  function updateArtOfferedByUser(userId, artId) {
    var deferred = q.defer();
    model.findById(userId, function(err, user) {
      if(err) {
        deferred.reject(err);
      } else {
        user.artOffered.push(artId);
  ...
  // Update order information for buyer
  function updateOrderInfoForUser(userId, orderId) {
    var deferred = q.defer();
    model.findById(userId, function(err, user) {
      if(err) {
        deferred.reject(err);
  ...
  function updateSalesInfoForUser(userId, orderId) {
    var deferred = q.defer();
    model.findById(userId, function(err, user) {
      if(err) {
        deferred.reject(err);
